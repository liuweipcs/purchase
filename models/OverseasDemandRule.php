 ...
 */
class OverseasDemandRule extends BaseModel
{
    /**
     * @inheritdoc
     */
    ...
    */
    ...
    public static function updateSummary($supplierSkus,$warehouseCode,$transport){
        ...
        foreach ($datas as $data){
            ...
            // 取SKU默认供应商里面的 退税信息
            $data->supplier_code = $product_quote->suppliercode;
            $data->pur_ticketed_point = $product_quote->pur_ticketed_point;
            $data->is_back_tax = $product_quote->is_back_tax == 1 ? 1 : 2;
...
//        $create_time = isset($timeData['update_time'])&&!empty($timeData['update_time']) ? $timeData['update_time'] : $timeData['create_time'];
    ...
    public static function agreeUpdateOrder($model, $product_quote)
    {
        ...
        $purchase_model = PurchaseOrder::find()->alias("a")
            ...
            ->where(['a.operation_type'=>2]) //运营方式：2自运营供
            ->andwhere(['a.purchase_type'=>2])
            ->andwhere(['a.supplier_code'=>$model->supplier_code]) //供应商编码
            ->andwhere(['a.warehouse_code'=>$model->purchase_warehouse])//仓库编码
            ->andwhere(['a.is_drawback'=>$model->is_back_tax == 1 ? 2 : 1]) //是否退税
            ->andWhere(['c.transport_style'=>$model->transport_style]) //运输方式(1空运2海运3铁路)
            ->andWhere("c.agree_time > '2018-08-29 10:00:00'") //需求同意时间
            ->andWhere("c.demand_status = 1") //等待采购询价
            ->one();
        ...
        if ($purchase_model) {
            ...
            if ($item_model) {
                $item_model->qty += $model->purchase_quantity;
                $item_model->ctq += $model->purchase_quantity;
        ...
        //更新需求推送状态
        PlatformSummary::updateAll(['is_push'=>0],['demand_number'=>$model->demand_number,'is_push'=>1]);
    }
}
