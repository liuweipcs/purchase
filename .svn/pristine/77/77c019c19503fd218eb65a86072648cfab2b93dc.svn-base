...
Modal::end();
?>
<h4><span class="glyphicon glyphicon-heart" style="color: red" aria-hidden="true"></span>温馨小提示:1,execl导入存在着乱码,请导入cvs格式的文件2,请先添加产品然后再选择仓库供应商</h4>
<div class="purchase-order-form">
    ...
    <div class="row">
        <div class="col-md-2">
            <?= $form->field($ordermodel, 'warehouse_code')->dropDownList(BaseServices::getWarehouseCode(),['prompt' => '请选仓库']) ?>
        </div>
        <div class="col-md-2">
        ...
        <table class="table table-bordered">
            ...
            <?php foreach($temporay as $k=>$v){?>
                <tr>
                    ...
                    <td><?=$v->title?:PurchaseHistory::getField($v->sku,'product_name')?></td>
                    <td><?=PurchaseHistory::getField($v->sku,'supplier_name')?></td>
                    <td><input  type="text" name="PurchaseOrder[items][<?=$k?>][purchase_price]" value="<?=$v->purchase_price?:PurchaseHistory::getField($v->sku,'purchase_price')?>"></td>
                    <td><input  type="number" name="PurchaseOrder[items][<?=$k?>][purchase_quantity]" value="<?=$v->purchase_quantity?:PurchaseHistory::getField($v->sku,'purchase_quantity')?>"></td>
                </tr>
            <?php } ?>
            </tbody>
