...
<div class="my-box" style="border: 2px solid #3c8dbc; margin-bottom: 10px;">
    ...
    <h5><label class="label label-info">合同信息</label></h5>

    <div class="col-md-1">
        <label>采购来源</label>
        <input type="text" class="form-control" value="合同采购" disabled>
        <input type="hidden" name="PurchaseOrder[source]" value="1"><!-- 国内合同 -->
    ...
    <div class="col-md-1">
        <label>是否中转</label>
        <?php
        if($model->is_transit == 0) {
            $a = '否';
        } else {
            $a = '是';
        }
        ?>
        <input type="text" name="PurchaseOrder[is_transit]" class="form-control" value="<?= $a ?>" disabled>
...
<div class="my-box" style="border: 1px solid red;margin-bottom: 10px;">
    ...
    <label>是否加急：</label>
    <input type="radio" name="PurchaseOrder[is_expedited]" value="1" checked> 不加急
    <input type="radio" name="PurchaseOrder[is_expedited]" value="2"> 加急

    <?php if($order->purchase_type != 1):
        $order->is_transit = 1;
        if(empty($order->is_drawback)) {
            $order->is_drawback = 1;
        }
        ?>

        <label>是否退税：</label>
        <select id="purchaseorder-transit_warehouse" class="is_drawback" name="PurchaseOrder[is_drawback]">
            <?php
