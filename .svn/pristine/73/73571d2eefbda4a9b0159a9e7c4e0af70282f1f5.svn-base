                ...
                'value' => function($model, $url, $key) {
                    ...
                    if($model->is_drawback == 2) {
                        ...
                        $data .= '<p>采购单号：'.Html::a($model->pur_number, ['view', 'id' => $model->pur_number],[
                            ...
                            ]).$check_html.'<strong style="margin-left:10px;color: green;">T</strong></p>';
    ...
    'columns' => [
        ...
        [
            ...
            'value' => function($model, $url, $key) {
                ...
                $data = '<p>'.PurchaseOrderServices::getPurchaseStatus($model->purchas_status).'</p>';
                $checkExist = \app\models\SupplierCheck::find()->where(['like','pur_number',$model->pur_number])->andWhere(['check_type'=>2])->andWhere('status<>3')->exists();
                $check_html='';
                if($checkExist){
                    $check_html='<sup style="color: red">验</sup>';
                }
