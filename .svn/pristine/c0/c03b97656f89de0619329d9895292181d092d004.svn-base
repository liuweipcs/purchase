            ...
            <div class="fg">
                ...
                <?php if($rpt == 0): ?>
                    ...
                    <select class="payType" name="payType">
                        <?php if($orderInfo['account_type'] == 2): ?>
...
$(function() {
    ...
    //账期订单请款方式只能为请款方式，请款数量、金额计算
    var payType_len = $(".payType").find("option").length;
    if(payType_len==1){
        var payType = "";
        $.each($(".payType").find("option"),function(k,v){
            payType = $(this).val();
        })
        if(payType==1){
            var attr = '';
...
/*        if(parseFloat(pay_price) > parseFloat(shengyu_money)) {
            layer.alert('请款金额已经超过了剩余金额，无法申请！');
            return false;
        }*/
