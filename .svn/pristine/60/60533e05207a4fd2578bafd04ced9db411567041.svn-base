            ...
            <table class="table table-bordered">
                ...
                foreach($vb->purchaseOrderItems as $k=> $v){
                    ...
                    <tr>
                        ...
                        <!--产品代码-->
                        <td><?=Html::a($v->sku, Yii::$app->params['SKU_ERP_Product_Detail'].$v->sku,['target'=>'blank'])/*.Html::a('',['product/viewskusales', 'sku' =>$v->sku], ['class' => "glyphicon glyphicon-signal b", 'style'=>'float: right', 'title' => '销量统计','data-toggle' => 'modal', 'data-target' => '#created-modal3',])*/ ?><a href="<?=$v->getSkuPurchaseLink()?>" title='' target='_blank'><i class='fa fa-fw fa-internet-explorer'></i></a></td>
                        <!--建议数量-->
                        <td style="width:50px;"><?=$v->qty?></td>
                        <!--产品名-->
                        <td  width="200" style="word-break:break-all;"  class="td1" onmouseover="this.className='td2'" onmouseout="this.className='td1'"><?=$v->name?></td>
                        <!--可用库存-->
                        ...
                        <!--税点-->
                        <td><input type="text" style="width:20px;<?php if($vb->is_drawback == 1){echo 'display:none';}?>" class="input-tax" name="PurchaseOrderTaxes[taxes][<?=$k?>][taxes]" readonly="readonly" value="<?php echo $v->pur_ticketed_point?>" data-point="<?php echo $v->pur_ticketed_point?>" required>%</td>
...
//                                'value' => date('Y-m-d',time()),
    ...
    <div class="form-group">
        <?= Html::submitButton($vb->isNewRecord ? Yii::t('app', '创建') : Yii::t('app', '确认'), ['class' => ($vb->isNewRecord ? 'btn btn-success' : 'btn btn-primary') . ' fba_submit',]) ?>
    </div>
