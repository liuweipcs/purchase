...
?>
    <div class="panel panel-default">
        ...
        <div class="panel-footer">
            ...
            if(Helper::checkRoute('/overseas-purchase-order/create-compact')) {
                echo Html::button('创建合同', ['class' => 'btn btn-success all-info fba-create-compact']);
            }

            if(Helper::checkRoute('/fba-purchase-order/compact-list')) {
                echo '<a href="/fba-purchase-order/compact-list" class="btn btn-info">合同列表</a>';
            }

            if(Helper::checkRoute('/purchase-order/print-data')) {
                echo Html::a('打印采购单', ['print-data'], ['class' => 'btn btn-success print','id'=>'submit-audit','target'=>'_blank']);
            }
            ?>
        </div>
    </div>
...
$(function() {
    ...
    $(document).on('click','#order-export',function() {
        ...
        if(str ==''){
            ...
            var summary_index = summary.lastIndexOf("，");
            summary = summary.substring(summary_index+1,summary.length);
            var value = summary.replace(/[^0-9]/ig,""); 
            if(confirm("总共 "+value+" 条数据，导出耗时较长，确认导出？")){
                $(this).attr('href',"{$exportUrl}?source="+source+"&purNumber="+str);
            }else{
                return false;
            }
        }else{
