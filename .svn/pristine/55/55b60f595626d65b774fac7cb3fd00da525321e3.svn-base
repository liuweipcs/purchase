...
<table class='table table-hover table-bordered table-striped' >
    ...
    <tr>
        ...
        <td>
            ...
            <?= $val['refund_amount'] > 0 ? '退款金额: ' . $val['refund_amount'] . '<br>' : '' ?>
            <?= $val['handle_type'] == 3 ? '退货数量: ' . $val['bad_products_qty'] . '<br>' : '' ?>
            <?= $val['handle_type'] == 4 ? '换货数量: ' . $val['bad_products_qty'] . '<br>' : '' ?>
        </td>
        <td>
            <?=Html::textarea("PurchaseQcAudit[{$val['id']}][note_audit]", $val['note_audit'])?></br>
            <?=Html::input('hidden',"PurchaseQcAudit[{$val['id']}][pur_number]", $data[0]['pur_number'])?></br>
