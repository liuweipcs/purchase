...
<div class="purchase-order-form">
        ...
        if($PurchaseOrderItems){
            foreach($PurchaseOrderItems as $v){
                ...
                <tr>
                    ...
                    <td><?=$v->sku?></td>
                    <td><?=$v->ctq?></td>
                    <td><?=\app\models\PurchaseOrderCancelSub::getCancelCtq($v->pur_number,$v->sku); ?></td>
                    <td><?=$v->price?></td>
                    <td><?=$v->rqy?></td>
    ...
    <table class="table table-hover">
        ...
        <tr>
            <?php
            $arr =['6'];
            if(in_array($model->refund_status,$arr)){
                ?>
            <td >
                <label><input type="radio" name="PurchaseOrder[refund_status]"   checked="checked" value="4">退款金额</label>
                <label><input type="text" name="PurchaseOrder[money1]" placeholder="请输入退款金额" value="<?=PurchaseRefunds::getRefundsAmount($pur_number); ?>"></label>
            </td>
            <?php }?>
        </tr>
        </tbody>
    ...
    <div class="form-group " style="clear: both">
        <input type="hidden" id="PurchaseOrder-refund_status" class="form-control" name="PurchaseOrder[refund_status]" value="" style="display:none">
...
$(document).on('click', '.btn-success', function () {
        $('#PurchaseOrder-refund_status').attr('value',6);
    });
     $(document).on('click', '.btn-warning', function () {
        $('#PurchaseOrder-refund_status').attr('value',7);
    });
