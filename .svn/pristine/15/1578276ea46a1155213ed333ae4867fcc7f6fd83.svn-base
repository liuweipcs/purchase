...
/*  public $p_start_time;
    ...
    public function rules()
    {
        return [
            [['id', 'pur_number', 'requisition_number', 'supplier_code',
                ...
                'compact.pur_number','created_at1','supplier_special_flag'
            ], 'safe'],
            [['pay_price'], 'number'],
        ];
    }
    ...
    {
        // 添加关联字段到可搜索属性集合
        return array_merge(parent::attributes(), ['compact.pur_number','items.sku']);
    }
    ...
    public function search($params, $purchase_type = null)
    {
        ...
        $query->andFilterWhere(['like', 'requisition_number', trim($this->requisition_number)])
    ...
    public function search2($params)
    {
        ...
        $query//->andFilterWhere(['like', 'pur_purchase_order_pay.pur_number', trim($this->pur_number)])
            ->andFilterWhere(['like', 'pur_purchase_order_pay.requisition_number', trim($this->requisition_number)])
            ->andFilterWhere(['like', 'pur_purchase_order_pay.supplier_code', trim($this->supplier_code)])
            ->andFilterWhere(['between', 'pur_purchase_order_pay.application_time', !empty($this->start_time)?$this->start_time:date('Y-m-d H:i:s',strtotime("-3 month")), !empty($this->end_time)?$this->end_time:date('Y-m-d H:i:s',time())]);
    ...
    public function search3($params, $platform = null)
    {
        ...
        if($platform == 'ABD') {
            ...
            if ($this->source == 2) {
                $query->innerJoinWith('purchaseOrder AS order');
            } else {
                $query->innerJoinWith('compactPurchaseOrder AS order');
            }
            $query->andFilterWhere(['=', 'order.is_new', 2]);
            $query->groupBy(['pur_purchase_order_pay.pur_number','pur_purchase_order_pay.requisition_number']);
        }
