...
class OverseasPurchaseOrderController extends BaseController
{
    ...
    public function executeCompactPayment($data)
    {
        ...
        $payinfo['money'] = $payinfo['money']-$cancel_price; //应请款金额
        if ($payinfo['money']<=0) exit("申请尾款请款金额不能大于应请款金额：应请款金额=原支付尾款金额-取消金额");

        // 本次申请金额 + 已申请金额
        $m3 = $m1 + $payinfo['money'];
        if(bccomp($m3, $m2, 2) == 1) exit('你的申请金额已经超出了合同总金额');
    ...
    public function actionPayment()
    {
        ...
        if($request->isPost) {
            ...
            if($cpn) { // 合同付款流程
                ...
                $cancel_price = PurchaseOrderCancel::getCompactCancelPrice($cpn);
    ...
    public function actionWritePayment()
    {
        ...
        if($request->isPost) {
            ...
            if($tid) {
                ...
                $account_type = in_array($model->pay_name,$self_pay_name_array) ? 2 : ($is_drawback==1 ? 2 : 1);//通过是否退税判断银行卡类型
                $is_drawback  = in_array($model->pay_name,$self_pay_name_array) ? 1 : $is_drawback;
                $account = SupplierPaymentAccount::find()->where(['supplier_code' => $model->supplier_code, 'status'=>1,'account_type'=>$account_type])->one();
    ...
    public function actionAddPurchaseNote()
    {
        ...
        try {
            if ( $model->load(Yii::$app->request->post()) && $model->save(false))
            {
                ...
                $models = PurchaseNote::getPurchaseNote($id);
...
//                    PurchaseDemand::UpdateOne($order['pur_number']);
    ...
    {        
        ...
        $cell_value = ['序号','图片','采购单号','采购仓库','中转仓库','采购员','采购日期','SKU','货品名称','采购单价','采购数量','实际应付货款','供应商名称','付款状态','采购状态','审核时间','订单号','物流单号','入库数量', '备注','采购异常回复','销售反馈','预计到货时间','采购开票点','是否退税','实际已付金额'];
        ...
        $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('AA3','入库单号');
        $objectPHPExcel->getActiveSheet()->getColumnDimension('AB')->setWidth(15);
        $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('AB3','良品上架数量');
        $objectPHPExcel->getActiveSheet()->getColumnDimension('AC')->setWidth(15);
        $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('AC3','入库时间');
        $objectPHPExcel->getActiveSheet()->getColumnDimension('AD')->setWidth(15);
        $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('AD3','订单状态');
        ...
        foreach ( $model as $v )
        {
            ...
            foreach ($v['purchaseOrderItems'] as $c=>$vb)
            {
                ...
                $objectPHPExcel->getActiveSheet()->setCellValue('E'.($n+4) ,BaseServices::getWarehouseCode($v['transit_warehouse']));
                $objectPHPExcel->getActiveSheet()->setCellValue('F'.($n+4) ,$v['buyer']);
                $objectPHPExcel->getActiveSheet()->setCellValue('G'.($n+4) ,$v['created_at']);
                $objectPHPExcel->getActiveSheet()->setCellValue('H'.($n+4) ,$vb['sku']);
                $objectPHPExcel->getActiveSheet()->setCellValue('I'.($n+4) ,$vb['name']);
                $objectPHPExcel->getActiveSheet()->setCellValue('J'.($n+4) ,$vb['price']);
                $objectPHPExcel->getActiveSheet()->setCellValue('K'.($n+4) ,$vb['ctq']);
                ...
                $objectPHPExcel->getActiveSheet()->setCellValue('L'.($n+4) ,round($pay,2));
                ...
                $objectPHPExcel->getActiveSheet()->setCellValue('N'.($n+4) ,$pay_status);

                $purchas_status = !empty($v['purchas_status'])?strip_tags(PurchaseOrderServices::getPurchaseStatus($v['purchas_status'])):'';
                $objectPHPExcel->getActiveSheet()->setCellValue('O'.($n+4) ,$purchas_status);
                $objectPHPExcel->getActiveSheet()->setCellValue('P'.($n+4) ,$v['audit_time']);

                $findone=PurchaseOrderOrders::findOne(['pur_number'=>$v['pur_number']]);
                $objectPHPExcel->getActiveSheet()->setCellValue('Q'.($n+4) ,!empty($findone) ? $findone->order_number : '');//订单号
                $objectPHPExcel->getActiveSheet()->setCellValue('R'.($n+4) ,!empty($v['orderShip']) ? $v['orderShip']['express_no'] : '');

                $results = WarehouseResults::getResults($vb['pur_number'],$vb['sku'],'instock_user,instock_date,arrival_quantity');
                $objectPHPExcel->getActiveSheet()->setCellValue('S'.($n+4) ,!empty($results->arrival_quantity)?$results->arrival_quantity:'0');
                $objectPHPExcel->getActiveSheet()->setCellValue('T'.($n+4) ,$purchaseNotes); //备注
                ...
                $objectPHPExcel->getActiveSheet()->setCellValue('U'.($n+4) ,$abnomal_data); //采购异常回复
                $objectPHPExcel->getActiveSheet()->getStyle('U')->getAlignment()->setWrapText(true);
                ...
                $objectPHPExcel->getActiveSheet()->setCellValue('V'.($n+4) ,$fan_data); //销售反馈
                $objectPHPExcel->getActiveSheet()->setCellValue('W'.($n+4) ,!empty($v['date_eta']) ? $v['date_eta'] : ''); // 预计到货时间

                $point = PurchaseOrderTaxes::getABDTaxes($vb['sku'],$vb['pur_number']) . '%';
                $objectPHPExcel->getActiveSheet()->setCellValue('X'.($n+4) ,$point); // 采购开票点

                $is_drawback = !empty($v['is_drawback']) ? ($v['is_drawback']==2?'是' : '否') : '否';
                $objectPHPExcel->getActiveSheet()->setCellValue('Y'.($n+4) ,$is_drawback); // 是否退税
    ...
    //                        ->createCommand()->getSql();
                ...
                $objectPHPExcel->getActiveSheet()->setCellValue('Z'.($n+4) ,$pay_money); // 实际已付金额
                ...
                $objectPHPExcel->getActiveSheet()->setCellValue('AA'. ($n+4) ,$instock_info['receipt_number']);
                $objectPHPExcel->getActiveSheet()->setCellValue('AB'.($n+4) ,$instock_info['instock_qty_count']);
                $objectPHPExcel->getActiveSheet()->setCellValue('AC'.($n+4) ,$instock_info['instock_date']);
                $objectPHPExcel->getActiveSheet()->setCellValue('AD'.($n+4) ,$instock_info['purchas_status']);
        ...
        if($request->isPost) {
            ...
            if($system['tid'] > 0) {
                ...
                try {
                    ...
                    foreach($pos as $pur_number) {
                        $exist = PurchaseCompactItems::find()->where(['pur_number'=>$pur_number,'bind'=>1])->exists();
                        if($exist){
                            throw new Exception('数据异常'.$pur_number.'已经生成了合同号');
                        }
                        $items[] = [
            ...
            if($tid > 0 && $_ratio && $ids) {
                ...
                if ($platform == 3) {
                    ...
                    $data = array_merge($data, ['settlement_ratio'=>$result['message'],'supplier_name'=>$result['supplier_name']]);
                }
                $data['referrer'] = Yii::$app->request->referrer;
        ...
        if($request->isPost) {
            ...
            try {
                ...
                if($a && $b && $c) {
                    ...
                    $t->commit();
                    Yii::$app->getSession()->setFlash('error','对不起，提交失败'.$a.$b.$c);
                    return $this->redirect(['index']);
                }
            } catch(\Exception $e) {
                $t->rollBack();
                Yii::$app->getSession()->setFlash('error','对不起，提交失败'.$e->getMessage());
                return $this->redirect(['index']);
            }
        } else {
    ...
    public function actionCancellations()
    {
        if (Yii::$app->request->isPost)
        {
            ...
            try{
                ...
                $compactItemsInfo = PurchaseCompactItems::getCancelJudgeCompact($post_info['pur_number']);//合同单，只付了订金

                if ($price_res !== true && empty($compactItemsInfo) ) {
                    Yii::$app->getSession()->setFlash('warning',$price_res);
                    return $this->redirect(Yii::$app->request->referrer);
                }
