...
class FbaPurchaseOrderController extends BaseController
{
    ...
    public function actionPayment()
    {
        ...
        if($request->isPost) {
            if(!empty($request->post('Payment')['source']) && ($request->post('Payment')['source'] == 1) ) { //合同
                $data =  $this->executeCompactPayment($request->post('Payment'));
                if (empty($data['error'])) {
                } else {
    ...
    public function getCompactPayment($compact_number=null)
    {
        ...
        foreach ($orderModel as $key => $model) {
            ...
            $payInfo     = $pay_model->getPayDetail($model->pur_number);
            $orderInfo   = $order_model->getOrderDetail($model->pur_number, $payInfo['skuNum']);
    ...
    public function executeCompactPayment($data)
    {
        ...
        // 获取申请金额
        $pay_ratio = isset($data['pay_ratio']) ? $data['pay_ratio'] : 0; //付款类别 0默认 10对私付运费 11合同全额付款 12合同定金 13合同中款 20合同尾款
        ...
        foreach ( $purNumber as $num )
        {
            ...
            if(!empty($datas)){
                ...
                foreach($datas as $val){
                    ...
                    $objectPHPExcel->getActiveSheet()->setCellValue('A'.($n+4) ,PurchaseOrder::getBuyer($val->pur_number));
                    ...
                    $objectPHPExcel->getActiveSheet()->setCellValue('P'.($n+4) ,$instock_info['receipt_number']);
                    $objectPHPExcel->getActiveSheet()->setCellValue('Q'.($n+4) ,$instock_info['instock_qty_count']);
                    $objectPHPExcel->getActiveSheet()->setCellValue('R'.($n+4) ,$instock_info['instock_date']);
                    $objectPHPExcel->getActiveSheet()->setCellValue('S'.($n+4) ,$instock_info['purchas_status']);
                ...
                $objectPHPExcel->getActiveSheet()->setCellValue('J'.($b+4) ,!empty($pur->purchaseOrderPayType) ? $pur->purchaseOrderPayType->freight : 0) ;
                if ($pur->purchas_status !=1) {
                ...
                $objectPHPExcel->getActiveSheet()->setCellValue('M'.($b+4) ,!empty($pur->warehouse_code)?BaseServices::getWarehouseCode($pur->warehouse_code):'');
