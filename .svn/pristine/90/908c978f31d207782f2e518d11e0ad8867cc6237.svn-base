...
$handler_type = !empty($model->handler_type) ? $model->handler_type : 2;
?>
<?php $form = ActiveForm::begin(['id' => 'handler-form']); ?>
...
<div class="box box-success">
    ...
    <div class="box-body">
        ...
        <div class="form-group">
            <label>处理类型</label>
            <br/>
            <input type="radio" name="handler_type" value="2" <?php if($handler_type == 2){ ?> checked <?php } ?>> 次品退货
            <input type="radio" name="handler_type" value="3" <?php if($handler_type == 3){ ?> checked <?php } ?>> 次品转正
            <input type="radio" name="handler_type" value="5" <?php if($handler_type == 5){ ?> checked <?php } ?>> 不做处理
        </div>
