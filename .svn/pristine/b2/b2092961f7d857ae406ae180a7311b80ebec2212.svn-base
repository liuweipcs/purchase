...
use app\models\PurchaseSuggestHistorySearch;
 ...
 * @desc 历史采购建议
 */
class PurchaseSuggestHistoryController extends BaseController
{
    ...
    {
        $searchModel = new PurchaseSuggestHistorySearch();
        $map=Yii::$app->request->queryParams;
        Yii::$app->session->set('get_parameter', $map);
        ...
        //产品状态统计
        /*$status['0']=PurchaseSuggestHistory::getStatus(0);
        $status['1']=PurchaseSuggestHistory::getStatus(1);
        $status['2']=PurchaseSuggestHistory::getStatus(2);
        $status['3']=PurchaseSuggestHistory::getStatus(3);
        $status['4']=PurchaseSuggestHistory::getStatus(4);
        $status['5']=PurchaseSuggestHistory::getStatus(5);
        $status['6']=PurchaseSuggestHistory::getStatus(6);
        $status['7']=PurchaseSuggestHistory::getStatus(7);
        $status['8']=PurchaseSuggestHistory::getStatus(8);
        $status['9']=PurchaseSuggestHistory::getStatus(9);
        $status['10']=PurchaseSuggestHistory::getStatus(10);
        $status['11']=PurchaseSuggestHistory::getStatus(11);
        $status['12']=PurchaseSuggestHistory::getStatus(12);
        $status['14']=PurchaseSuggestHistory::getStatus(14);
        $status['15']=PurchaseSuggestHistory::getStatus(15);
        $status['16']=PurchaseSuggestHistory::getStatus(16);
        $status['total']=PurchaseSuggestHistory::getStatus(99,'1');*/
...
//            exit;
    ...
    protected function actionSavePurchase($res)
    {
        ...
        foreach ($res['PurchaseOrder']['items'] as $val)
        {
            ...
            if($sb)
            {
                ...
                //回写采购建议，标识已经生产采购单
                $model_suggest           = new PurchaseSuggestHistory();
                $where['sku']            = $val['sku'];
                $where['warehouse_code'] = $model_order->warehouse_code;
                //$where['is_purchase']    = 'Y';

                $id = $model_suggest->find()->where($where)->one();
                $id->load(['PurchaseSuggestHistory' => ['is_purchase' => 'N','state'=>1,'demand_number' =>$model_order->pur_number]]);
    ...
    public function actionExport()
    {
        ...
        $get_parameter = Yii::$app->session->get('get_parameter');

        if (empty($get_parameter['PurchaseSuggestHistorySearch'])) {
            ...
            //以写入追加的方式打开
            //时间
            if (!empty($get_parameter['PurchaseSuggestHistorySearch']['end_time'])) {
                $start_time = $get_parameter['PurchaseSuggestHistorySearch']['start_time'];
                $end_time = $get_parameter['PurchaseSuggestHistorySearch']['end_time'];
            } else {
            ...
            //sku
            if (trim($get_parameter['PurchaseSuggestHistorySearch']['sku']) != '') {
                $query->andWhere(['in', 'sku',trim($get_parameter['PurchaseSuggestHistorySearch']['sku'])]);
            }
            //处理状态
            if ($get_parameter['PurchaseSuggestHistorySearch']['state'] == '') {

            } else {
                $query->andWhere(['in', 'state',$get_parameter['PurchaseSuggestHistorySearch']['state']]);
            }
            //供应商
            if ($get_parameter['PurchaseSuggestHistorySearch']['supplier_code']!='') {
                $query->andWhere(['in', 'supplier_code',$get_parameter['PurchaseSuggestHistorySearch']['supplier_code']]);
            }
            //是否欠货
             if($get_parameter['PurchaseSuggestHistorySearch']['left'] == 1)
             {
                 $query->andFilterWhere(['<','left_stock',0]);
             } elseif($get_parameter['PurchaseSuggestHistorySearch']['left'] == 2){
                 $query->andFilterWhere(['>=','left_stock',0]);
             }
            //产品状态
            if($get_parameter['PurchaseSuggestHistorySearch']['product_status'] != '')
            {
                $query->andFilterWhere(['in','product_status',$get_parameter['PurchaseSuggestHistorySearch']['product_status']]);
            }
            //采购员
            if ($get_parameter['PurchaseSuggestHistorySearch']['buyer_id'] != '') {
                if (is_numeric($get_parameter['PurchaseSuggestHistorySearch']['buyer_id'])) {
                    $query->andFilterWhere(['in','buyer_id',$get_parameter['PurchaseSuggestHistorySearch']['buyer_id']]);
                } else {
                    $group=[
                        'g1'=>'1',
                        'g2'=>'2',
                    ...
                    $gid=$group[$get_parameter['PurchaseSuggestHistorySearch']['buyer_id']];
...
//        Vhelper::dump(count($model));
        ...
        $cell_value = ['序号', '采购员', 'SKU', '产品状态', '产品名称', '供应商', '单价', '日均销量', '可用库存', '在途库存', '欠货', '采购数量', '创建时间', '处理状态'];
        foreach ($cell_value as $k => $v) {
            $objectPHPExcel->setActiveSheetIndex(0)->setCellValue(chr($k+65) . '3',$v);
        }

        //设置表头居中
        $objectPHPExcel->getActiveSheet()->getStyle('A3:N3')->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        //设置数据水平靠左和垂直居中
        ...
        foreach ( $model as $v )
        {
            ...
            $objectPHPExcel->getActiveSheet()->setCellValue('D' . ($n + 4), isset($v['product_status'])?\app\services\SupplierGoodsServices::getProductStatus($v['product_status']):"未知");
            $objectPHPExcel->getActiveSheet()->setCellValue('E' . ($n + 4), $v['name']);
            $objectPHPExcel->getActiveSheet()->setCellValue('F' . ($n + 4), $v['supplier_name']);
            ...
            $objectPHPExcel->getActiveSheet()->setCellValue('G' . ($n + 4), $price);
            $objectPHPExcel->getActiveSheet()->setCellValue('H' . ($n + 4), $v['sales_avg']);
            $objectPHPExcel->getActiveSheet()->setCellValue('I' . ($n + 4), $v['available_stock']);
            $objectPHPExcel->getActiveSheet()->setCellValue('J' . ($n + 4), $v['on_way_stock']);

            $objectPHPExcel->getActiveSheet()->setCellValue('K' . ($n + 4),  $v['available_stock']+$v['on_way_stock']+$v['left_stock']);
            $objectPHPExcel->getActiveSheet()->setCellValue('L' . ($n + 4), $v['qty']);
            $objectPHPExcel->getActiveSheet()->setCellValue('M' . ($n + 4), $v['created_at']);
            $objectPHPExcel->getActiveSheet()->setCellValue('N' . ($n + 4), isset($v['state'])?(\app\services\PurchaseOrderServices::getProcesStatus()[$v['state']]):'');
...
//        $objectPHPExcel->getActiveSheet()->getColumnDimension('E')->setAutoSize(true); //设置列与数据的宽相等
        ob_end_clean();
        ob_start();
    ...
    public function actionQtyView()
    {
        ...
        $model_suggest= PurchaseSuggestHistory::find()->where(['sku'=> $sku])->one();
    ...
    public function actionGetHistoryNote(){
        ...
        $noteDatas = PurchaseSuggestNote::find()
            ...
            ->andWhere(['or',['and',['<=','create_time',$suggest_create_time],['or',['>=','update_time',$suggest_create_time],['update_time'=>null]]],['and',['>=','create_time',$suggest_create_time],['<=','create_time',date('Y-m-d 23:59:59',strtotime($suggest_create_time))]]])
            ->asArray()->all();
        if(empty($noteDatas)){
