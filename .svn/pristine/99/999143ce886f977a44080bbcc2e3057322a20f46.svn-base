    ...
    'columns'=>[
        ...
        [
            ...
            'value' => function($model) {
                ...
                if($form_id) {
                    $data .= Html::a('<span class="glyphicon glyphicon-envelope" title="查看付款申请书"></span>', ['/purchase-compact/show-form', 'id' => $model->id], [
                        'title' => Yii::t('app', '查看付款申请书'),
                        'class' => 'show-form',
                        'data-toggle' => 'modal',
        ...
        [
            ...
            'format' => 'raw',
            'value' => function($model) {
                return PurchaseOrderPay::getPrice($model,false,1);
            }
        ],
        [
            'label'=>'供应商名称',
        ...
        [
            ...
            'buttons'=>[
                'view' => function($url,$model,$key) {
                    return Html::a('<i class="glyphicon glyphicon-eye-open"></i> 查看',['/overseas-purchase-order-pay/view','id'=>$model->id],[
                        'title'=>'查看',
                ...
                'audit' => function($url, $model, $key) {
                    if($model->pay_status == 10) {
                        return Html::a('<i class="glyphicon glyphicon-wrench"></i> 经理审核',['/overseas-purchase-order-pay/compact-audit', 'id' => $model->id], [
                            'title' => '经理审核',
                ...
                'submit' => function($url, $model, $key) {
                    if($model->pay_status == -1 && Yii::$app->user->identity->id == $model->applicant) {
                        return Html::a('<i class="glyphicon glyphicon-open"></i> 提交',['/overseas-purchase-order-pay/submit','id' => $model->id], [
                            'title' => '提交',
