...
$this->title = '国内仓订单合同-通用版';
?>
<?php if(isset($print)): ?>

<table class="table table-bordered" style="width: 750px;background-color: #fff;margin-bottom: 0px;">
    <tr>
...
<table class="my-table2">
    ...
    <tr>
        ...
        <td colspan="6">
            ...
            <div class="sbox">
                ...
                <p>东莞市塘厦镇科苑城科苑大道16号君盈购物广场    邱谭胜   18873346252</p>
    ...
    <?php
    $realMoney = $total_money + $freight['freight'] - $freight['discount'];
    ?>
            ...
            <?php
                ...
                if(count($_ratio) > 1):
                    ...
                    if(count($_ratio) == 2) {
                        ...
                        $dj = ($total_money*(int)$djRatio)/100 - $freight['discount'];  // 订金 = 产品总额*首款百分比-优惠
                        $wk = ($total_money*(int)$wkRatio)/100 + $freight['freight'];;
                        $wwk = $wk;
                        ...
                        $dj = ($total_money*(int)$djRatio)/100 - $freight['discount'];  // 订金 = 产品总额*首款百分比-优惠
                        $wk = ($total_money*(int)$wkRatio)/100;
                        $wwk = ($total_money*(int)$wwkRatio)/100 + $freight['freight'];
                    }

                ?>

                    <label><?= $djRatio ?>订金：</label>
                    <input type="text" name="Compact[dj_money]" value="<?= $dj ?>" style="width: 60px;" readonly>

                    <label><?= $wkRatio ?>尾款：</label>
                    <input type="text" name="Compact[wk_money]" value="<?= $wk ?>" style="width: 60px;" readonly>

                    <label>尾款总额：</label>
                    <input type="text" name="Compact[wk_total_money]" value="<?= $wwk ?>" style="width: 60px;" readonly>

                <?php endif; ?>

            <label>运费：</label>
            <input type="text" name="Compact[freight]" value="<?= $freight['freight'] ?>" style="width: 60px;" readonly>

            <label>优惠金额：</label>：
            <input type="text" name="Compact[discount]" value="<?= $freight['discount'] ?>" style="width: 60px;" readonly>
...
<table class="my-table2">
    ...
    <div style="width: 100%;text-align: center;padding-top: 10px;">
        ...
        <input type="hidden" name="Compact[supplier_name]" value="<?= $data['supplier']->supplier_name; ?>">
        <input type="hidden" name="Compact[supplier_code]" value="<?= $data['supplier']->supplier_code; ?>">
