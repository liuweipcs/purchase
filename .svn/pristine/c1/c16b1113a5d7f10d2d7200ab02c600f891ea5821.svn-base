...
<div class="row">
    ...
    <!--<div class="col-md-4"><?/*= $form->field($model, 'esupplier_name')->textInput(['maxlength' => true,'placeholder'=>'yibai network']) */?></div>-->
   ...
   <!-- <div class="col-md-4"><?/*= $form->field($model, 'merchandiser')->dropDownList(BaseServices::getEveryOne(), ['prompt' => '请选择跟单员']) */?></div>-->
   ...
   <!-- <div class="col-md-4"><?/*= $form->field($model, 'commission_ratio')->textInput(['maxlength' => true]) */?></div>-->
    ...
    <div class="col-md-4"><?/*= $form->field($model, 'area')->dropDownList(BaseServices::getCityList($model->city),['prompt'=>'--请选择区--',]) */?></div>-->
...
if($model->pay) {
    ...
    foreach ($model->pay as $k => $v) {
        ...
        $pay .='<td>'.$form->field($model, "payment_method")->dropDownList(['2'=>'在线','3'=>'银行卡'],['class' => 'form-control pay_method','name'=>"SupplierPaymentAccount[$k][payment_method]",'value'=>$v->payment_method,'disabled'=>'disabled'])->label(false).'</td>';


        if ($v->payment_method==2)
        {

            $pay .= '<td>'.$form->field($model, "payment_platform")->dropDownList(['1'=>'paypal','2'=>'财付通','3'=>'支付宝','4'=>'快钱','5'=>'网银'],['class' => 'form-control payment_platform','name'=>"SupplierPaymentAccount[$k][payment_platform]",'value'=>$v->payment_platform])->label(false).'</td>';
            '</td>';

        } else{
           
            $pay .= '<td>'.$form->field($model, "payment_platform_bank")->dropDownList(\app\services\SupplierServices::getPayBank(),['class' => 'form-control pay_bank','name'=>"SupplierPaymentAccount[$k][payment_platform_bank]",'value'=>!empty($v->payment_platform_bank)?$v->payment_platform_bank:''])->label(false);
             $pay.= $form->field($model, "payment_platform_branch")->textInput(['class' => 'form-control pay_bank','placeholder'=>'请录入支行名称','name'=>"SupplierPaymentAccount[$k][payment_platform_branch]",'value'=>$v->payment_platform_branch])->label(false).'</td>';

        }
        $pay .='<td>'.$form->field($model, "account")->textInput(["maxlength" => true,"placeholder"=>"必填项",'name'=>"SupplierPaymentAccount[$k][account]",'value'=>$v->account])->label(false).'</td>';
        $pay .= '<td>'.$form->field($model, "account_name")->textInput(["maxlength" => true,"placeholder"=>"必填项",'name'=>"SupplierPaymentAccount[$k][account_name]",'value'=>$v->account_name])->label(false).'</td>';
        $pay .= '<td>'.$form->field($model, "status")->dropDownList(['1'=>'可用','2'=>'不可用'],['class' => 'form-control pay_method','name'=>"SupplierPaymentAccount[$k][status]",'value'=>$v->status])->label(false).'</td>';
...
if($model->contact)
{
    foreach ($model->contact as $k => $v)
    {
        ...
        $contact.= '<td > '.$form->field($model, "contact_number")->textInput(['class' => 'form-control ','placeholder'=>'联系电话','value'=>!empty($v->contact_number)?$v->contact_number:'','name'=>"SupplierContactInformation[$k][contact_number]"])->label('').' </td >';
        $contact.='<td > '.$form->field($model, "contact_fax")->textInput(['class' => 'form-control ','placeholder'=>'Fax','value'=>!empty($v->contact_fax)?$v->contact_fax:'','name'=>"SupplierContactInformation[$k][contact_fax]"])->label('').' </td >';
        ...
        $contact.='<td > '.$form->field($model, "english_address")->textInput(['class' => 'form-control ','placeholder'=>'英文联系地址','value'=>!empty($v->english_address)?$v->english_address:'','name'=>"SupplierContactInformation[$k][english_address]"])->label('').' </td >';
        $contact.='<td > '.$form->field($model, "contact_zip")->textInput(['class' => 'form-control ','placeholder'=>'联系邮编','value'=>!empty($v->contact_zip)?$v->contact_zip:'','name'=>"SupplierContactInformation[$k][contact_zip]"])->label('').' </td >';
        $contact.='<td > '.$form->field($model, "qq")->textInput(['class' => 'form-control ','placeholder'=>'QQ','value'=>!empty($v->qq)?$v->qq:'','name'=>"SupplierContactInformation[$k][qq]"])->label('').' </td >';
        $contact.='<td > '.$form->field($model, "micro_letter")->textInput(['class' => 'form-control ','placeholder'=>'微信','value'=>!empty($v->micro_letter)?$v->micro_letter:'','name'=>"SupplierContactInformation[$k][micro_letter]"])->label('').' </td >';
        $contact.='<td > '.$form->field($model, "want_want")->textInput(['class' => 'form-control ','placeholder'=>'旺旺','value'=>!empty($v->want_want)?$v->want_want:'','name'=>"SupplierContactInformation[$k][want_want]"])->label('').' </td >';
        $contact.='<td > '.$form->field($model, "skype")->textInput(['class' => 'form-control ','placeholder'=>'Skype','value'=>!empty($v->skype)?$v->skype:'','name'=>"SupplierContactInformation[$k][skype]"])->label('').' </td >';

        //$contact.='<td > '.Html::button('删除', ['class' => 'btn btn-danger form-control','style'=>'margin-top:20px;']).' </td>';
...
$items = [
    ...
    [
        ...
        'content'=>$form->field($model, 'image_url')->widget(FileInput::classname(), ['options' => ['multiple' => true,'name'=>'SupplierImages[image_url]'],
            'pluginOptions' => [
                // 需要预览的文件格式
