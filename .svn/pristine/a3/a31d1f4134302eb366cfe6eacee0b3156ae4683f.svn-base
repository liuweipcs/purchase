...
//            ])->post($url);
    ...
    public function actionGetProductSupplier()
    {
        set_time_limit(0);
        //偏移量用于截取数组
        $offset = ApiPageCircle::find()->select('page')->where(['type'=>'PRODUCT_SUPPLIER_OFFSET'])->orderBy('id DESC')->scalar();
        if(!$offset){
            $offset = 0;
        }
        $sku = Yii::$app->request->getQueryParam('sku','');
        if(!empty($sku)){
            $data = Product::findBySql("SELECT t.sku,t.id FROM pur_product as t LEFT JOIN (SELECT * FROM pur_product_supplier WHERE is_supplier IN (1,2)) as a ON t.sku=a.sku WHERE  isNull(a.id) AND t.sku = $sku AND t.product_is_multi != 2 AND t.product_type !=2 AND t.product_status NOT IN (0,7) AND (t.create_time > '2017-01-01 00:00:00' OR (t.product_status != 1 AND t.create_time<'2017-01-01 00:00:00')) ORDER BY t.id ASC  ")
                ->asArray()
                ->all();
        }else{
            $data = Product::findBySql("SELECT t.sku,t.id FROM pur_product as t LEFT JOIN (SELECT * FROM pur_product_supplier WHERE is_supplier IN (1,2)) as a ON t.sku=a.sku WHERE  isNull(a.id) AND t.product_is_multi != 2 AND t.product_type !=2 AND t.product_status NOT IN (0,7) AND (t.create_time > '2017-01-01 00:00:00' OR (t.product_status != 1 AND t.create_time<'2017-01-01 00:00:00')) ORDER BY t.id ASC   limit $offset,50")
                ->asArray()
                ->all();
        }
        if(empty($data)){
            ...
            ApiPageCircle::insertNewPage($offset+50,'PRODUCT_SUPPLIER_OFFSET');
        }
        $this->getSupplierInfo(array_column($data,'sku'));
    }

    protected function getSupplierInfo($sku){
        $curl = new curl\Curl();
        ...
        if (!empty(json_decode($s)->data)) {
            foreach (json_decode($s)->data as $value) {
                ...
                if (!empty($supplier)) {
                    ...
                    $deleteSupplier = Supplier::find()->where(['supplier_name' => $value->provider,'status'=>[2,4]])->one();
...
//        }
    ...
    public function actionPushTaxPoint() {
        $datalist = ProductTicketedPointLog::find()->where(['is_push'=>0])->orderBy("id asc")->limit(100)->all();
        $params = $ids = [];
        ...
        //$pushUrl = 'http://local.wwwerp.com/services/products/product/getticketedpoint';
        $pushUrl = 'http://119.23.218.76/services/products/product/getticketedpoint';
        //$pushUrl = Yii::$app->params['ERP_URL'].'/services/products/product/updatesku';
        $curl = new curl\Curl();
        $s = $curl->setPostParams([
        ...
        if($s == '发生了错误') {
            ...
            $res = Json::decode($s);print_r($res);
            if(!empty($res)){
                echo ProductTicketedPointLog::updateAll(['is_push'=>1], ['in','sku',$res]);
