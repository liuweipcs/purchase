...
<div class="purchase-order-search">
    ...
    <input type="hidden" name="PlatformSummarySearch[type]" value="amount">
    <input type="hidden" name="PlatformSummarySearch[tab]" value="2">
    ...
    <div class="col-md-1">
        <?= $form->field($model, 'level_audit_status')->dropDownList(['0'=>'待同意','1'=>'同意','2'=>'驳回','3'=>'撤销','4'=>'采购驳回','5'=>'删除','6'=>'规则拦截','7'=>'待提交'],['prompt' => '请选择']) ?>
    </div>
    <div class="col-md-2">
...
$js = <<<JS
    $(function(){
        $("select[name='PlatformSummarySearch[product_category]']").show();
        $("select[name='PlatformSummarySearch[purchase_warehouse]']").show();
        $("select[name='PlatformSummarySearch[supplier_code]']").show();
    });
JS;
$this->registerJs($js);
?>
