 ...
 */
class ProductRepackageController extends BaseController
{
    const SKU_TYPE = 1;// 国内仓
    ...
    public function actionIndex()
    {
        ...
        $dataProvider = $searchModel->search1($params);
        $data = [
    ...
    public function actionAudit(){
        ...
        foreach($ids as $id){
            ...
            if($res){
                ...
                $productModel->is_repackage = ($audit_status == 1)?1:0;// 审核通过标记为包，审核不通过取消 包 标记
                $productModel->save(false);

                \app\api\v1\models\Product::skuWeightMarkUpdate(['sku' => $model->sku,'is_repackage' => ($audit_status == 1)?1:0]);// 更新是否是二次包装标记
            }
        }
    ...
    public function actionBatchDelete(){
        ...
        foreach($ids as $id){
            ...
            if($res){
                ...
                \app\api\v1\models\Product::skuWeightMarkUpdate(['sku' => $model->sku,'is_repackage' => 0]);// 更新是否是二次包装标记
            }
        }
    ...
    public function actionImport(){
        ...
        if(Yii::$app->request->isPost AND $_FILES){
            ...
            if (move_uploaded_file($tmp_name, $filePath)) {
                ...
                $exists_list = $success_list = $no_sku_list = [];// 已存在 成功 SKU不存在 个数
