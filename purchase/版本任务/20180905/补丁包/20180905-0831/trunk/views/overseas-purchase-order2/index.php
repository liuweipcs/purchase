...
<div class="panel panel-default" style="position:relative;z-index:10">
    <div class="panel-body">
        ...
        <div class="col-md-1">
            <?= $form->field($model, 'transport_style')->dropDownList(PurchaseOrderServices::getTransport(),['prompt'=>'全部'])->label('运输方式') ?>
        </div>
        <div class="col-md-1">
...
HTML;
    ...
    <div class="box-footer">
		...
		<?= Html::button('确认提交信息',['class'=>'btn btn-success btn-info-submit','data-url'=>'confirm-info']) ?>
		<?php endif; ?>
		<?php if (Helper::checkRoute('info-audit')) : ?>
        <?= Html::button('信息变更审核',['class'=>'btn btn-info btn-audit','data-url'=>'info-audit','data-toggle'=>'modal','data-target'=>"#create-modal"]) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('purchase-audit')) : ?>
        <?= Html::button('采购审核',['class'=>'btn btn-info btn-audit','data-url'=>'purchase-audit','data-toggle'=>'modal','data-target'=>"#create-modal"]) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('sale-audit')) : ?>
        <?= Html::button('销售审核',['class'=>'btn btn-info btn-audit','data-url'=>'sale-audit','data-toggle'=>'modal','data-target'=>"#create-modal"]) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('purchase-disagree')) : ?>
        <?= Html::button('驳回',['class'=>'btn btn-warning btn-note-submit','data-action'=>'purchase-disagree']) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('confirm-order')) : ?>
        <?= Html::button('生成进货单',['class'=>'btn btn-success btn-submit','data-action'=>'confirm-order']) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('change-buyer')) : ?>
        <?= Html::button('变更采购员',['class'=>'btn btn-warning btn-audit','data-url'=>'change-buyer','data-toggle'=>'modal','data-target'=>"#create-modal"]) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('add-note')) : ?>
        <?= Html::button('添加备注',['class'=>'btn btn-warning btn-note-submit','data-action'=>'add-note']) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('pay-apply')) : ?>
        <?= Html::button('申请请款',['class'=>'btn btn-success btn-submit','data-action'=>'pay-apply']) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('add-express-no')) : ?>
        <?= Html::button('录入物流单号',['class'=>'btn btn-success btn-audit','data-url'=>'add-express-no','data-toggle'=>'modal','data-target'=>"#create-modal"]) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('cancel-order')) : ?>
        <?= Html::button('作废订单',['class'=>'btn btn-danger btn-audit','data-url'=>'cancel-order','data-toggle'=>'modal','data-target'=>"#create-modal"]) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('cancel-audit')) : ?>
        <?= Html::button('审核作废订单',['class'=>'btn btn-success btn-audit','data-url'=>'cancel-audit','data-toggle'=>'modal','data-target'=>"#create-modal"]) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('purchase-abnormal-answer')) : ?>
        <?= Html::button('采购异常回复',['class'=>'btn btn-warning btn-note-submit','data-action'=>'purchase-abnormal-answer']) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('sale-feedback')) : ?>
        <?= Html::button('销售反馈',['class'=>'btn btn-warning btn-note-submit','data-action'=>'sale-feedback']) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('invoice')) : ?>
        <?= Html::button('开票',['class'=>'btn btn-warning btn-audit','data-url'=>'invoice','data-toggle'=>'modal','data-target'=>"#create-modal"]) ?>
        <?php endif; ?>
        <?php if (Helper::checkRoute('split-purchase')) : ?>
        <?= Html::button('拆分采购单',['class'=>'btn btn-success btn-audit','data-url'=>'split-purchase','data-toggle'=>'modal','data-target'=>"#create-modal"]) ?>
        <?php endif; ?>
...
$columns =  [
    ...
    'pur_number' => [
        ...
        'value'=> function($model){
            return getDivContent(!empty($model->pur_number) ? $model->pur_number : '',70);
        },
    ],
    'compact_number' => [
        'label'=>'合同号',
    ...
    'supplier_code' => [
        ...
        'value' => function($model) {
            return getDivContent(empty($model->supplier2) ? '' : $model->supplier2->supplier_name,140,'font-size:12px');
        }
    ],
    'purchase_quantity' => [
        'label' => '数量',
    ...
    'item_totalprice' => [
        ...
        'value' => function($model) {
            ...
            return getDivContent(round($price * $model->purchase_quantity,4),60);
        }
    ],
    'transit_number' => [
        'label' => '中转数',
    ...
    'pay_percent' => [
        ...
        'value' => function($model) {
            if ($model->demand_status == 1) {
                ...
                $string .= '<input style="width:140px" readonly type="text" name="settlement_ratio['.$model->demand_number.']" class="form-control settlement_ratio same-pur-number" data-pur-number="'.$model->pur_number.'" data-field="settlement_ratio" value="'.$model->settlement_ratio.'">';
        ...
        'value' => function($model) {
            if ($model->demand_status == 1) {
                return DateTimePicker::widget([
                    ...
                    'options' => ['placeholder'=>'','style'=>'width:150px','data-pur-number'=>$model->pur_number,'class'=>'same-pur-number','data-field'=>'date_eta'],
                    'pluginOptions' => [
                        'autoclose' => true,
                        'todayHighlight' => true,
    ...
    'demand_freight' => [
        ...
        'format' => 'raw',
        'value' => function($model) {
            return getDivContent(OverseasPurchaseOrderSearch::getDemandPayInfo($model->demand_number, 'freight'),40);
        }
    ],
    'freight_type' => [
        'label' => '运费计算方式',
    ...
    'demand_discount' => [
        ...
        'format' => 'raw',
        'value' => function($model) {
            return getDivContent(OverseasPurchaseOrderSearch::getDemandPayInfo($model->demand_number, 'discount'),50);
        }
    ],
    'purchase_acccount' => [
        'label' => '账号',
