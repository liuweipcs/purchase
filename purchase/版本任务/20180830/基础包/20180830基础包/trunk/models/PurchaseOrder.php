...
class PurchaseOrder extends \yii\db\ActiveRecord
{
    ...
    public static function createFbaPurOrder2($data)
    {
        ...
        foreach($data as $value) {
            ...
            if($supplier) {
                ...
                $code = $supplier->supplier_code.'_'.$value->purchase_warehouse.'_'.$value->is_transit.'_'.$value->transit_warehouse.'_'.$is_back_tax;
        ...
        try {
            ...
            foreach($list as $v) {
                ...
                $orderdata['purdesc']['is_back_tax']       = $v[0]['is_back_tax'] == 1 ? 1 : 2;
                $pur_number                                = self::Savepurdata($orderdata);
    ...
    // 根据订单号获取订单详情
    public function getOrderDetail($purNumber, $skuNum = [])
    {
        ...
        if($items) {
            ...
            foreach($items as $k => $v) {
                ...
                if($is_drawback&&$model->warehouse_code=='TS'&&$model->purchase_type==3){
                    ...
                    $price = $v['price']*$point/100 + $v['price'];
                }
    ...
    public static function getOrderSkuInfo($purNumber)
    {
        ...
        if($items) {
            foreach($items as $item) {
                ...
                $arr['shouhuo_num']    = $item['rqy'] ? $item['rqy'] : $arr['arrival_quantity'];   // 收货数量
                $arr['weidaohuo_num']  = $item['ctq'] - $arr['shouhuo_num'];                       // 未到货数量
                //如果是海外仓的，取到货数量
                $type = Vhelper::getNumber($purNumber);
                $arr['ruku_num']       = ($type==2)?$arr['arrival_quantity']:$arr['instock_qty_count'];                                // 入库数量
    ...
    public static function getCompactGeneralData($dd, $type = 2)
    {
        ...
        $supplierAccount = SupplierPaymentAccount::find()->where(['supplier_code' => $model->supplier_code])->asArray()->one();
    ...
    {
        $id_arr = explode(',', $ids);
        $sql = 'SELECT supplier_code,is_drawback,pay_type,account_type,shipping_method,settlement_ratio 
        ...
        return ['error'=>0, 'message' => $res['settlement_ratio'][0]];
    }
    /**
     * 根据对比结果，返回相应数据
     */
...
";
    ...
    //instock_date >= '2018-05-01 00:00:00' and instock_date < '2018-06-01 00:00:00' and 
    if (!empty($time)) {
...
where b.purchas_status in (7, 8) and b.pay_status in (5)";
    ...
    $sql .= ' GROUP BY a.pur_number, a.sku';
        ...
        if(false!==fopen($file,'w+')){ 
          file_put_contents($file,serialize($q));//写入缓存 
        }
    ...
    public static function getDetails($pur_number,$cancel_id=null)
    {
        ...
        if ($model->source == 1) {
            // 1合同 2网络【默认】 3账期采购
            $res['compact_number'] = $model->purchaseCompactItems->compact_number;
        }
