...
class PurchaseOrderSearch extends PurchaseOrder
{
    ...
    public function rules()
    {
        return [
            [['id'], 'integer'],
            [['warn_status','settlement','grade','pur_number', 'warehouse_code', 'supplier_code', 'pur_type', 'shipping_method', 'operation_type', 'creator', 'account_type', 'pay_type', 'currency_code','items.sku','buyer','purchas_status','refund_status','create_type','audit_return','created_at','singletype','reference','start_time','end_time','pur_type','is_arrival','complete_type','qc_abnormal_status','receiving_exception_status','code','ss.supplier_type','sku_type','merchandiser','buyer','supplier_name','pay_status','ship.express_no','page_size','sku','express_no','supplier_name','source','product_is_new','is_drawback','arrival_status','is_check_goods'], 'safe'],
        ];
    }
    ...
    {
        // 添加关联字段到可搜索属性集合
        return array_merge(parent::attributes(), ['items.sku','ss.supplier_type','ship.express_no','pro.product_is_new']);
    }
    /**
     * @inheritdoc
     */
...
//        $query->andFilterWhere(['!=','purchase_type','3']);
    ...
    {
        $query = PurchaseOrder::find()->where(['source' => 2,'pur_purchase_order.purchase_type' => 2]);
        ...
        if(empty($this->buyer)){
            ...
            $query->andFilterWhere(['in','buyer',array_column($user,'username')]);
        }
...
//        ->andFilterWhere(['like', 'pur_purchase_order.supplier_name', $this->supplier_name]);
    ...
    public function search10($params,$noDataProvider = false)
    {
        ...
        $query->where(['source' => 1,'pur_purchase_order.purchase_type' => 2]);
        $query->andWhere(['in', 'purchas_status', ['3', '5', '6', '7', '8', '9', '10']]);
