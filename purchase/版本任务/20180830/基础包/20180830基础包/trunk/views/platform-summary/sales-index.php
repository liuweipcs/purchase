...
<div class="purchase-order-index">
    ...
    <h4><p class="glyphicon glyphicon-heart pps" style="color: red" aria-hidden="true">温馨提示:</p><span style="color: red"><p class="pps">1.撤销的需求不可再还原,请看清楚了再撤销。驳回请重新修改,只有同意了才能创建采购单</p><p class="pps">2.备货金额大于等于10000或者备货数量大于等于100,需待采购审核</p></h4>
    <?= GridView::widget([
                ...
                'value'=>
                    function($model){
                        ...
                        //$data.= $model->product_category?'采购员：'.\app\models\PurchaseCategoryBind::getBuyer($model->product_category).'<br/>':'';
                          ...
                          $data.= '<span style="color:red">sku:'.Html::a($model->sku, Yii::$app->params['SKU_ERP_Product_Detail_Hide'].$model->sku,['target'=>'blank']).'</span><br/>';
...
//                          $data.= '<span style="color:#00a65a">供应商:'.BaseServices::getSupplierName($suppliercode).'</span></br>';
           ...
           /* [
            ...
            [
                ...
                'value'=>
                    function($model){
                        if($model->level_audit_status==0 && $model->init_level_audit_status==1){
                            return '待采购主管审核';
                        }
            ...
            [
                ...
                'dropdown' => false,
                'width'=>'180px',
                'template' => Helper::filterActionColumn('{update}{agree}{disagree}{init-agree}{init-disagree}'),
                'buttons'=>[
                    'update' => function ($url, $model, $key)
                    {
                        ...
                        if(Yii::$app->user->identity->username != '刘楚雯' && $model->init_level_audit_status == 1) {
                            return Html::a('<i class="fa fa-fw"></i>待审核');
                        }
                    },
                    'agree' => function ($url, $model, $key)
                    {
                        if(Yii::$app->user->identity->username != '刘楚雯' && $model->init_level_audit_status != 1) {
                            ...
                            if(!in_array($model->level_audit_status,$arr)) {
                                ...
                                return Html::a('<i class="fa fa-fw fa-check"></i>提交', ['agree', 'id' => $key,'page'=>$page], [
                                    'title' => Yii::t('app', '提交'),
                                    'class' => 'btn btn-xs red'
                                ]);
                            }
                    ...
                    'disagree' => function ($url, $model, $key)
                    {
                        if(Yii::$app->user->identity->username != '刘楚雯' && $model->init_level_audit_status != 1) {
                            ...
                            if(!in_array($model->level_audit_status,$arr)) {
                                $page=Yii::$app->request->get('page') ? Yii::$app->request->get('page') : 1;
                                return Html::a('<i class="fa fa-fw fa-close"></i>撤销', ['disagree', 'id' => $key,'page'=>$page], [
                                    'title'       => Yii::t('app', '撤销'),
