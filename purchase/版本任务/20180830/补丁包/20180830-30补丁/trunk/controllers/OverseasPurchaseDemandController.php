...
class OverseasPurchaseDemandController extends BaseController
{
    ...
    public function actionIndex()
    {
        ...
        if(isset($params['PlatformSummarySearch']['tab']) && $params['PlatformSummarySearch']['tab']==2){
            ...
            $amountInterceptData = $searchModel->searchAmountIntercept();
        }
        //7天3小时拦截数据
        if(isset($params['PlatformSummarySearch']['tab']) && $params['PlatformSummarySearch']['tab']==3){
            ...
            $sevendays3hoursData = $searchModel->search7days3hours();
        }
        //产品信息不全数据
        if(isset($params['PlatformSummarySearch']['tab']) && $params['PlatformSummarySearch']['tab']==4){
            ...
            $incompleteInfoData = $searchModel->searchIncompleteInfo();
        }
        ...
        return $this->render('index', [
            ...
            'amountTotal'=>count($amountInterceptData),//金额拦截总数
    ...
    public function actionCreate()
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            if(empty($product)||!in_array($product->product_status,[4,10,12,2,3,14,15,16,17,18,19])){
                Yii::$app->getSession()->setFlash('error','产品不存在或者产品不在如下属性(状态:拍摄中,修图中,编辑中,预上线,在售中,设计审核中,文案审核中,文案主管审核中,试卖编辑中,试卖在售中,试卖文案终审中)');
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $update_data_param = [
                ...
                'transit_number' => '中转数量',
                'transport_style' => ['运输方式', PurchaseOrderServices::getTransport()],
        ...
        if (Yii::$app->request->isPost && $_FILES)
        {
            ...
            while ($datas = fgetcsv($file)) {
                ...
                if(!empty($Name[$line_number][6]) && !is_numeric($Name[$line_number][6])){
                    switch (trim($Name[$line_number][6])){
                        case '海运散货':
                            $Name[$line_number][6]='56';
                            break;
                        case '空运散货':
                            $Name[$line_number][6]='57';
                            break;
                        case '快递':
                            $Name[$line_number][6]='58';
                            break;
                        case '陆运':
                            $Name[$line_number][6]='16162';
                            break;
                        case '铁路散货':
                            $Name[$line_number][6]='15275';
                            break;
                        case '海运整柜':
                            $Name[$line_number][6]='15274';
                            break;
                        case '空运整柜':
                            $Name[$line_number][6]='15272';
                            break;
                        case '铁路整柜':
                            $Name[$line_number][6]='15273';
                            break;
                        default:
                            $Name[$line_number][6]='';
                    }
                ...
                if(empty($product)||!in_array($product->product_status,[4,10,12,2,3,14,15,16,17,18,19])){
                    Yii::$app->getSession()->setFlash('error','产品不存在或者产品不在如下属性(状态:拍摄中,修图中,编辑中,预上线,在售中,设计审核中,文案审核中,文案主管审核中,试卖编辑中,试卖在售中,试卖文案终审中)--'.$sku,true);
                    return $this->redirect(['index']);
                }
                if(empty($Name[$line_number][6])){
                    Yii::$app->getSession()->setFlash('warning','请使用新的头程运输方式');
                    return $this->redirect(['index']);
                }
            ...
            foreach($ids as $v){
                ...
                if (strtotime($model->create_time)>strtotime('2018-05-08 23:59:59')){
                    ...
                    if($ruleVerify['status']=='error'){
                        ...
                        echo json_encode(['status'=>$ruleVerify['status'],'message'=>$ruleVerify['supplier_name'].'<br/>仓库：'.$warehouseName.'<br/>运输方式：'.$transport.'<br/>'.$ruleVerify['message']]);
                        Yii::$app->end();
                    }
                }
            }
            ...
            try{
                foreach ($updateForm as $data){
                    ...
                    Yii::$app->db->createCommand()->update(PlatformSummary::tableName(),
                        ['level_audit_status'=>3],
                        ['purchase_type'=>2,'demand_number'=>$data['demand_number'],'purchase_warehouse'=>$warehouse_code])->execute();
                ...
                $tran->rollBack();
                Yii::$app->getSession()->setFlash('error','批量撤销失败');
            }
            return $this->redirect(Yii::$app->request->referrer);
        }
    }
