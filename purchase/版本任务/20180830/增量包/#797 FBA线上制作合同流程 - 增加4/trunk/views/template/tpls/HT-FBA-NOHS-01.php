...
<?php if(isset($print)): ?>
    ...
    <table class="my-table2">
		<tr><th colspan="12" style="text-align: center;">YIBAI TECHNOLOGY LTD</th></tr>
        <tr>
            <th colspan="6">甲方</th>
            <th colspan="6">乙方</th>
        </tr>
        ...
        ?>
                ...
                foreach($items as $k=>$item):
                    ...
                    <tr>
                        ...
                        <?php endif; ?>
                        <td><?= $data['audit_time'] ?></td> <!-- 采购时间 -->
                        <td><?= $data['date_eta'] ?></td> <!-- 交货时间/预计到货时间 -->
                    </tr>
                <?php endforeach; ?>
            <?php endforeach; ?>
       ...
       <tr>
		...
		<tr>
            <th>送货方式</th>
            <td colspan="11"><?= $model->note_other?></td>
        </tr>
		<tr>
            <th>总金额</th>
...
<?php else: ?>
    ...
    <div class="compact-content">
        ...
        <table class="my-table2">
            <tr><th colspan="12">YIBAI TECHNOLOGY LTD</th></tr>
            <tr>
                <th colspan="6">甲方</th>
                <th colspan="6">乙方</th>
            </tr>
            ...
            <tr>
                ...
                <th>地  址</th>
                <td colspan="5">
					<textarea name="Compact[y_address]" style="width: 100%;border: 0px;" readonly><?= !empty($data['supplierContent']) ? $data['supplierContent']->chinese_contact_address : ''; ?></textarea>
                </td>
            </tr>
            <tr>
                <th rowspan="2">授权代表(采购)</th>
            ...
            foreach($data['purchaseOrderItems'] as $pur_number => $items):
            	$jiaohuo_time = ($jiaohuo_time-strtotime($data['date_eta']))>0?$jiaohuo_time:strtotime($data['date_eta']); //计算最晚交货日期
                foreach($items as $k=>$item):
                    $img = Html::img(Vhelper::downloadImg($item['sku'], $item['product_img'], 2), ['width' => '60px', 'height' => '60px']);
                    $total_money += $item['price']*$item['ctq'];
                    $ctq_total += $item['ctq'];
                    ...
                    <tr>
                        ...
                        ?>
                            ...
                            <td rowspan="<?= count($items) ?>" style="vertical-align: middle;text-align: center;"><?= $discount ?></td> <!-- 优惠额 -->
                        <?php endif; ?>
                        <td><?= date('Y-m-d', strtotime($data['audit_time'])) ?></td> <!-- 采购时间 -->
                        <td><?= date('Y-m-d', strtotime($data['date_eta'])) ?></td> <!-- 交货时间/预计到货时间 -->
                    </tr>
                <?php endforeach; ?>
            <?php endforeach; ?>
            <tr>
...
东莞市塘厦镇科苑城科苑大道16号君盈购物广场一楼FBA仓101 <?= $data['buyer'] ?>转赵容17817013672
</textarea>
                </td>
            </tr>
            <tr>
            ...
            <?php $plan = PurchaseCompact::PaymentPlan3($data['settlement_ratio'], $total_money, $freight_total, $discount_total); ?>
            <tr>
                <th>付款方式??</th>
                <td colspan="11">
            ...
            <?php
            $texts = [];
			$tests[1] = "、付款方式：" . SupplierServices::getSettlementMethod($data['account_type']);
			$tests[2] = "、货款应在货物送达甲方指定地点并经甲方验收合格，且甲方收到乙方正式等额发票（包括但不限于增值税发票，采购订单合同或者形式发票等）后开始启动付款期，乙方出示大货图发于甲方安排发货";
			$tests[3] = "、甲方向乙方预付订单30%的货款，进行生产。";
			$tests[4] = "、大货完成后支付70%的尾款，尾款支付后当天算起，乙方必须在三天内发货并出示物流信息。乙方出示大货图发于甲方安排发货。";
			$tests[5] = "、货款应在货物送达甲方指定地点并经甲方验收合格，且甲方收到乙方正式等额发票（包括但不限于增值税发票，采购订单合同或者形式发票等）后开始启动70%尾款付款期，乙方出示大货图发于甲方安排发货";
			$tests[6] = "、大货完成后，拍大货图由甲方确认后申请全款；";
			$tests[7] = "、全款支付后当天算起，乙方必须在三天内发货并出示物流信息。";
			$tests[8] = "、乙方未按时交货，自逾期起需每日向甲方支付全款金额的0.8‰作为违约滞纳金。其它未尽事宜，大家友好协商解决。";
			$tests[9] = "、如甲方已支付预付款项，乙方则需配合甲方处理出口退税的相关事宜。";
            ...
            } elseif( ($data['settlement_ratio'] == '10%+30%+60%') && ($data['account_type']==2) && ($data['pay_type']==3) ) {
            	//预付全款 款到发货，银行卡转账 100% 4.7.8.9.10
            	//$data['account_type'] 2
            	//$data['pay_type'] 3
                $payment_explain = Vhelper::pluck($tests, [4, 7, 8, 9, 10]);
            } else {
...
<b>二、条码要求：</b>
1、所有产品条码需保证能被扫描设备正确读取数据；
2、条码由买方提供的电子档直接打印或甲方直接提供，不可随意更改。<br />
<b>三、装箱要求：</b><br />
...
五、乙方有义务按甲方要求提供商品的材质成分、物料清单、必要的出口认证资料（包括但不仅限于CE、FCC、FDA、FSE、ROHS、C-TICK、HDMI、UN38.3、En/en、CSA、IC、EK、MTC、SASO、IRAM、NOM）、说明书等，乙方提供及全力配合后期的证询并协助甲方进行清关，如因资料不全、产品质量问题或虚假认证等问题导致无法正常清关，乙方承担全部责任（包括但不限于货物成本、运输成本、报关费用、销毁费用等）。<br />
...
一、甲方所下达订单，乙方需在交货期前5天完成并通知甲方采购员，以便甲方确认是否按排人员前去乙方验货。所有甲方人员验货时，乙方均需提供OQC检验报表以及其他相关二、买方验货前，卖方须保证所有产品务必按该订单要求生产包装完毕，卖方进行自检合格后，同时附上装箱清单(装箱清单的SKU顺序须与订单上的SKU顺序保持一致）、自检合三、甲方有权对乙方的生产现场、生产流程、作业方式等进行审核，并提出改善建议；乙方对甲方的质量稽查须予以支持和配合，不允许有以任何形式隐瞒产品质量的现象。<br/ >
