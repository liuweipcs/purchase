    ...
    <div class="row">
        ...
        <div class="col-md-4">
            ...
            <div class="form-group field-purchaseorder-is_transit">
                ...
                <input type="hidden" name="PurchaseOrdersV2[is_transit]" value="">
                <input type="hidden" name="PurchaseOrdersV2[purchase_type]" value="" class="purchase_type">
                <div id="purchaseorder-is_transit">
                    <label><input id="check-radio1" type="radio" name="PurchaseOrdersV2[is_transit]" value="1"> 需要中转</label>
                    <label><input id="check-radio2" type="radio" name="PurchaseOrdersV2[is_transit]" value="0" checked=""> 直发</label>
                </div>
        ...
        <table class="table table-bordered">
            ...
            <?php foreach($temporay as $k=>$v){?>
                <tr>
                    <td><input type="text" name="PurchaseOrdersV2[items][<?=$k?>][sku]" value="<?=$v->sku?>" readonly></td>
                    <td><?=$v->title?:\app\models\ProductDescription::getFiled($v->sku,'title')?></td>
                    <td><?=PurchaseHistory::getField($v->sku,'supplier_name')?></td>
                    <td><input  type="text" name="PurchaseOrdersV2[items][<?=$k?>][purchase_price]" value="<?=$v->purchase_price?'10':PurchaseHistory::getField($v->sku,'purchase_price')?>"></td>
                    <td><input  type="number" name="PurchaseOrdersV2[items][<?=$k?>][purchase_quantity]" value="<?=$v->purchase_quantity?'10':PurchaseHistory::getField($v->sku,'purchase_quantity')?>"></td>
                </tr>
            <?php } ?>
            </tbody>
