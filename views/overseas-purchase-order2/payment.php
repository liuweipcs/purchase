...
$url_export  = Url::toRoute('payment-export');// 请款单导出URL
...
HTML;
    ...
    <div class="box-footer">
        ...
        <?= Html::button('导出',['class'=>'btn btn-primary btn-export','data-url'=>'payment-export']) ?>
...
$columns =  [
    ...
    'supplier_code' => [
        ...
        'format' => 'raw',
        'value' => function($model) {
            $supplierName = PurchaseOrder::getSupplierName($model->pur_number);
            if (empty($supplierName)) $supplierName = $model->purchaseCompact[0]->purchaseOrder->supplier_name;
            $sub_html = \app\models\SupplierSearch::flagCrossBorder(true,$model->supplier_code);
            return getDivContent($supplierName.$sub_html,100);
        }
    ],
    'contact_person' => [
        'label' => '联系人',
    ...
    'pay_price' => [
        ...
        'value' => function($model) {
            ...
            if ($model->source == 1) {
                return getDivContent(round($model->pay_price, 3).$check_priv,100);
            } else {
                return getDivContent(round(PurchaseOrderPay::getPrice($model,true), 2).$check_priv,100);
            }
        }
    ],
    'real_pay_price' => [
...
$(function() {
    ...
    $(".btn-export").click(function(){
        ...
        var url = '{$url_export}';
        if (data.length == 0){
            layer.confirm('您没有选择任何记录，将按查询条件导出<br/>继续导出？', {
            btn : [ '确定', '取消' ]//按钮
            }, function(index) {
               window.open(url);
               layer.close(index);
...
function change_pull_right_summary(){
    var right_html = $(".summary").html();
   var now_html = "总金额："+'{$totalPayPrice}&nbsp;&nbsp;&nbsp;'+right_html;
   $(".summary").html(now_html);
}

$(document).ready(function () {
    change_pull_right_summary();// 自动刷新总金额
