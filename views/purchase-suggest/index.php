...
//                ],
                ...
                [
                    ...
                    'pageSummary'=>true,
                    'value'=>function($data){
                        $pur_price=\app\models\PurchaseOrderItems::findOne(['sku'=>$data-> sku])['price'];
                        $pro_price=$data->product_cost;
                ...
                [
                    ...
                    'value'=> function($data){
                        ...
                        return Html::input('text','suggest-note',!empty($data->purchaseSuggestNote)?$data->purchaseSuggestNote->suggest_note:'',['readonly'=>'readonly','sku'=>$data->sku,'warehouse_code'=>$data->warehouse_code,'style'=>'width:200px','note'=>!empty($data->purchaseSuggestNote)?$data->purchaseSuggestNote->suggest_note:'']) . '<br />' . $create_time . '<br />' . $creator;
                    }
                ],
                [
                    'label'=>'历史采购信息',
    ...
    $(function(){
        ...
        $("a#create-purchase").click(function(){
            ...
            if(ids==''){
                ...
                if($(this).hasClass("pp"))
                    {
                        url = '/purchase-suggest/create-purchase';
                    }
                url=url+'?ids='+ids;
                $(this).attr('href',url);
                $.post(url, {},
        ...
        $('#batch-create-purchase').click(function() {
            ...
            //有勾选生成单号
            if(ids!=''){
                $.post('/purchase-suggest/lead-suggest', {ids: ids},function(data){
                    $('#create-purchase-modal').find('.modal-body').html(data);
                });
                ...
                $.post('/purchase-suggest/lead-suggest', function(data){
                    $('#create-purchase-modal').find('.modal-body').html(data);
                });
            }
        });
        ...
        $("a#edit-buyer").click(function(){
            ...
            if(ids==''){
               ...
               $.get('/purchase-suggest/editbuyer', {id: ids},function (data) {
                        $('#create-purchase-modal').find('.modal-body').html(data);
    ...
    $("input[name='suggest-note']").change(function(){
     ...
     $('#export-csv').click(function() {
            ...
            /*if(ids==''){
                ...
                window.location.href='/purchase-suggest/export-csv?ids='+ids;
            /*}*/
     })        
JS;
$this->registerJs($js);
