...
<?php if(isset($print)): ?>
    ...
    <table class="my-table2" autosize="1">
        <tr>
            <th>采购单号</th>
        ...
        $pur_numbers = array_keys($products);
        ...
        foreach($products as $pur_number => $items):
            ...
            endforeach;
                ...
                foreach($items as $k=>$item):
                    ...
                    $point_price = ((float)$rate*$item['price'])/100 + $item['price'];
                ...
                ?>
                    ...
                    <tr>
                        ...
                        <td style="text-align: center;"><?= $point_price ?></td>
                        <td style="text-align: center;"><?= $item['ctq'] ?></td>
                        <td style="text-align: center;"><?= $point_price*$item['ctq'] ?></td>
        ...
        <tr>
            <th rowspan="2">备注</th>
            <td>运费</td>
            <td colspan="6"><?= $model->note_freight ?></td>
        </tr>
        <tr>
            <td>其它</td>
            <td colspan="7"><?= $model->note_other ?></td>
        </tr>
        <tr>
            <th>送货方式</th>
            <td colspan="7"><?= $model->ship_method ?></td>
        </tr>

        <tr>
            <th>收货地址</th>
            <td colspan="7"><?= $model->shouhuo_address ?></td>
        </tr>
        <tr>
            <th>总金额</th>
            <td><?=$totalprice;?></td>
            <td colspan="6" style="font-size: 20px;">
                <?php $plan = \app\models\PurchaseCompact::PaymentPlan3($model->settlement_ratio, $totalprice, $model->freight, $model->discount, 2);?>
        ...
        <tr>
            <th>付款说明</th>
            <td colspan="7"><?= $model->payment_explain ?></td>
        </tr>
        <tr>
            <th>合作要求</th>
            <td colspan="7"><?= $model->hezuo_reqiure ?></td>
        </tr>
        <tr>
            <th>汇款信息</th>
            <td colspan="7" style="font-size: 20px;"><?= $model->huikuan_information ?></td>
        </tr>
        <tr>
            <th colspan="8" style="text-align: center;">合约要求</th>
        </tr>
        <tr>
            <td colspan="8"><?= $model->heyue_require ?></td>
        </tr>
        <tr>
            <th colspan="8" style="text-align: center;">质检要求</th>
        </tr>
        <tr>
            <td colspan="8"><?= $model->zhijian_require ?></td>
        </tr>
        ...
        <table class="my-table2">
            ...
            foreach($data['purchaseOrderItems'] as $pur_number => $items):
                ...
                foreach($items as $k=>$item):
                    $img = Html::img(Vhelper::downloadImg($item['sku'], $item['product_img'], 2), ['width' => '60px', 'height' => '60px']);
                    $rate = PurchaseOrderTaxes::getABDTaxes($item['sku'],$item['pur_number']);
                    $ratio_price = ((float)$rate*$item['price'])/100 + $item['price'];
                    $total_money += $ratio_price*$item['ctq'];
                    ?>
                    <tr>
                        <?php if($k == 0): ?>
                        ...
                        <td><?= $item['name'] ?></td>
                        <td style="text-align: center;"><?= $item['price'] ?></td>
                        <td style="text-align: center;"><?= $ratio_price ?></td>
                        <td style="text-align: center;"><?= $item['ctq'] ?></td>
                        <td style="text-align: center;"><?= $ratio_price*$item['ctq'] ?></td>
                        <td style="text-align: center;"><?= $img ?></td>
                    </tr>
                <?php endforeach; ?>
            <?php endforeach; ?>
            ...
            <tr>
                <th rowspan="2">备注</th>
                <td>运费</td>
                <td colspan="6"><textarea name="Compact[note_freight]" rows="3" style="width: 100%;"></textarea></td>
            </tr>
            <tr>
                <td>其它</td>
                <td colspan="7"><textarea name="Compact[note_other]" rows="3" style="width: 100%;"></textarea></td>
            </tr>
            <tr>
                <th>送货方式</th>
                <td colspan="7"><textarea name="Compact[ship_method]" rows="3" style="width: 100%;">乙方需运送至甲方公司指定仓库地址</textarea></td>
            </tr>
            ...
            $realMoney = $total_money - $freight['discount'];
            ?>
            ...
            <tr>
                ...
                <td colspan="7">
                    ...
                    $plan = \app\models\PurchaseCompact::PaymentPlan3($data['settlement_ratio'], $total_money, $freight['freight'], $freight['discount'], 2);
...
3、在合格质量水平栏找出要求的AQL值，在没有特别要求下，以AQL值为2.5作检验标准；2.5对应的检验标准为：AC(可接受)=2，Re(不可接受)=3。<br/>
        ...
        <div style="width: 100%;text-align: center;padding-top: 10px;">
            ...
            <input type="hidden" name="Compact[is_drawback]" value="2">
            ...
            <a href="javascript:void(0)" class="btn btn-default" onclick="javascript :history.back(-1)">返回上一步</a>
