        ...
        <table class="table table-bordered table-condensed">
            ...
            if($payInfo['auditor']):
                ...
                <tr style="background-color: <?= $bg ?>">
                    ...
                    <th>审核备注</th>
                    <td><?= !empty($payInfo['review_notice']) ? $payInfo['review_notice'] : ''; ?></td>
                </tr>
            <?php endif; ?>
            <?php
            if($payInfo['approver']):
                ...
                <tr style="background-color: <?= $bg ?>">
                    ...
                    <th>审批备注</th>
                    <td><?= !empty($payInfo['processing_notice']) ? $payInfo['processing_notice'] : ''; ?></td>
                </tr>
            <?php endif; ?>
            <?php
            if($payInfo['payer']):
...
<!--
如果支付方式是银行卡转账和富友则显示银行卡信息
-->
<?php if(isset($payInfo['pay_type'])&&in_array($payInfo['pay_type'],[3,5])){?>
    <div class="row">
        <?php
            ...
            $accountType = in_array($payInfo['pay_name'],$self_pay_name_array) ? 2 : (isset($orderInfo['is_drawback'])&&$orderInfo['is_drawback']==1 ? 2 : 1);//通过是否退税判断银行卡类型
            $bankCardInfo = \app\models\SupplierPaymentAccount::find()
                ->where(['supplier_code'=>$payInfo['supplier_code']])
