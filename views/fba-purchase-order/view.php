...
//请款数量
$sku_list = PurchaseOrderPayDetail::getSkuList(['pur_number'=>$model->pur_number],true);
?>
<?php
    ...
    <table class="table table-bordered">
        ...
        if($model->purchaseOrderItemsCtq){
            foreach($model->purchaseOrderItemsCtq as $v){
                ...
                //已请款数量
                $is_pay = false; //是否已申请付款
                $payModel = !empty($model->purchaseCompactItems->purchaseOrderPay)?$model->purchaseCompactItems->purchaseOrderPay:$model->purchaseOrderPays;
                if (!empty($payModel)) {
                    foreach ($payModel as $pk=>$pv) {
                        if (in_array($pv->pay_status, [2,4,5,6,10,13])) $is_pay=true;
                    }
                }

                $qingkuan_num = isset($sku_list[$v->sku])?$sku_list[$v->sku]: $v->ctq;
                $qingkuan_num = $is_pay?$qingkuan_num:0;
                ...
                <tr>
                    ...
                    <td><?php
                        $html = Html::a($v->sku, Yii::$app->params['SKU_ERP_Product_Detail'].$v->sku,['target'=>'blank']).\app\models\ProductRepackageSearch::getPlusWeightInfo($v->sku,true);;
