        ...
        'columns' => [
            ...
            [
                ...
                'value'=> function($model, $key, $index, $column){
                    ...
                    $data .= \app\models\ProductRepackageSearch::getPlusWeightInfoByPurNumber($model->pur_number,isset($model->purchaseOrderItems)?$model->purchaseOrderItems:null);// 根据采购单SKU展示 加重标记
                    $cpn = !empty($model->purchaseCompact) ? $model->purchaseCompact->compact_number : '';
                    $data .= "<br />合同号：{$cpn}";
                    $sub_html = !empty($model->supplier_name)?\app\models\Supplier::flagCrossBorder(true,null,$model->supplier_name):'';
                    $data.="<br/>".Yii::t('app','供应商').':'.$model->supplier_name.$sub_html."<br/>";
                    ...
                    $data .= "<a href='compact-list/?compact_number={$cpn}' title='去合同列表查看' target='_blank'><span class='glyphicon glyphicon-tags' style='color: #f2adb1;margin-right: 10px;'></span></a>";
                    
                    $data .= "<a href='/purchase-compact/view?cpn={$cpn}' title='去合同详情查看' target='_blank'><span class='glyphicon glyphicon-eye-open' style='color: #f2adb1;margin-right: 10px;'></span></a>";
            ...
            [
                ...
                "format" => "raw",
                'value' => function($model) {
                    if (empty($model->pay_status)) return '';
            ...
            [
                ...
                'value'=> function($model, $key, $index, $column){
                    ...
                    $diff_time = (strtotime($model->date_eta)-time())/86400;// 相差的天数（小数）
                    if($diff_time >= 3){
                        $data .= '<span style="color:green">到货时间:'.$model->date_eta.'</span><br/>';
                    }else{
            ...
            [
                ...
                'buttons'=>[
                    ...
                    'cancellations' => function ($url, $model, $key) {
                        ...
                        $arr = [3, 7, 8];
                        $not_pay_status = [2,4,10];
                        if ( ($model->buyer==$username || BaseServices::getIsAdmin()) && in_array($model->purchas_status, $arr) && !in_array($model->pay_status,$not_pay_status)) {
