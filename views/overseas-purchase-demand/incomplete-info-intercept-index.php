...
<span  class="tab_index" tab_index="<?= $tab_index?>"></span>
    ...
    <?php foreach ($v as $key=>$value){?>
        <?php
            if($index == 1){
                $data = "<h4 style='float: left;padding-right: 50px;'>全选: <input type='checkbox' id='theadInp2'></h4>";
                echo $data;
            }
            $index++;
        ?>
        ...
        <?php
        $ids_list = [];
        foreach($value as $ids_value){
            $ids_list[] = $ids_value->id;
        }
        $ids_list = implode(',',$ids_list);
        foreach ($value as $kes=>$vs){?>
            <tr>
                ...
                <?php if($kes==0){ ?>
                <td rowspan="<?=count($value)?>" style="text-align: center">
                    <?php
                    echo \yii\helpers\Html::a('修改', ['batch-update'], ['class' => 'batch-update','data-toggle' => 'modal', 'data-target' => '#create-modal','suplier_code'=>$k,'warehouse_code'=>$key,'num'=>count($value),'ids_list'=>$ids_list]);
                    ?>
                    <?php
                    echo \yii\helpers\Html::a('撤销', ['batch-revoke'], ['class' => 'batch-update','data-toggle' => 'modal', 'data-target' => '#create-modal','suplier_code'=>$k,'warehouse_code'=>$key,'num'=>count($value),'ids_list'=>$ids_list]);
                    ?>
                </td>
                <?php }?>
            </tr>
...
$(function() {
    ...
    $("#theadInp2").on('click', function() {
        $("#type_4 input:checkbox").prop("checked", $(this).prop('checked'));
    })
